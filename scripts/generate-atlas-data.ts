 ...
 * ship several hundred KB of endpoints it can never load.
 *
 * Paths are resolved from the current working directory, so a consumer runs it
 * from its own root: `tsx node_modules/interlocking/scripts/generate-atlas-data.ts`.
 */
...
const BRANCH = 'main';
const CONCURRENCY = 8;
const OUTPUT_PATH = path.join(process.cwd(), 'public', 'atlas-feeds.json');
/** Sibling checkout of the calling app, used in preference to the GitHub API. */
const LOCAL_ATLAS_PATH = path.join(process.cwd(), '..', 'transitland-atlas');
...
async function fetchJson<T>(url: string): Promise<T> {
  const res = await fetch(url, {
    headers: {
      ...
      'User-Agent': 'generate-atlas-data',
    },
  });
  if (!res.ok) {
...
function humanizeFeedId(feedId: string): string {
  ...
  if (segments.length > 1 && GEOHASH_SEGMENT.test(segments[0])) {
    segments.shift();
  }
  return segments.join('-').replace(/~/g, ' ').replace(/_/g, ' ').trim();
}
...
function buildRows(docs: SourceDoc[]): AtlasRow[] {
  ...
  for (const { origin, dmfr } of docs) {
    for (const feed of dmfr.feeds ?? []) {
      if (!USABLE_SPECS.has(feed.spec ?? 'gtfs')) {
        continue;
      }
      ...
      if (!scheduledUrl && !vehiclesUrl && !tripUpdatesUrl && !alertsUrl) {
        continue;
      }
...
async function mainLocal(atlasPath: string) {
  ...
  console.log(
    `Reading ${dmfrFiles.length} local DMFR files from ${atlasPath}...`
  );

  const docs = await runConcurrently(
    dmfrFiles,
    CONCURRENCY,
    async (filePath) => {
      try {
        ...
        return {
          origin: originFromFilename(filePath),
...
async function mainRemote() {
  ...
  const docs = await runConcurrently(
    ...
    async (repoPath) => {
      ...
      if (done % 100 === 0) {
        process.stdout.write(`  ${done}/${dmfrFiles.length}\r`);
      }
