...
export type AlertLevel =
  'feed' | 'agency' | 'route-type' | 'route' | 'trip' | 'stop' | 'route-stop';
...
export function alertsForStop(
  ...
  return all(session).filter((r) =>
    selectors(r).some((e) => e.stopId === stopId)
  );
}
...
export function isActiveNow(
  ...
  return periods.some(
    (p) =>
      (p.start === undefined || p.start <= nowSeconds) &&
      (p.end === undefined || p.end >= nowSeconds)
  );
}
...
export function translations(
  ts: TranslatedString | null | undefined
): Translation[] {
