...
import type { TripUpdate, VehiclePosition } from './rt-types';
 ...
 * - `derived` — it sent neither, and the stop was worked out from the trip's
 *   predictions. This one is an inference and has to be declared.
 */
...
export class RtIndex {
  ...
  constructor(session: FeedSession, nowSeconds = Date.now() / 1000) {
    ...
    for (const update of session.tripUpdates) {
      this.ingestUpdate(update, feed);
    }
    for (const vehicle of session.vehicles.values()) {
      this.ingestVehicle(vehicle, feed);
    }
  ...
  private ingestUpdate(update: TripUpdate, feed: GTFSScheduled | null): void {
    ...
    for (const stu of update.stopTimeUpdate ?? []) {
      ...
      const stopId =
        stu.stopId ??
        (sequence !== undefined
          ? times?.find((t) => t.stop_sequence === sequence)?.stop_id
          : undefined);
      const stopRelationship = presentNumber(stu, 'scheduleRelationship');
      if (stopRelationship !== undefined) {
        bump(this.relationships.stopTimes, stopRelationship);
      }
    ...
    for (const p of predictions) {
      push(this.predictionsByStop, p.stop_id, p);
    }
  }
  ...
  private ingestVehicle(
    ...
    const routeId =
      (vehicle.tripId && feed?.trips.get(vehicle.tripId)?.route_id) ||
      vehicle.routeId;
    if (routeId) {
      push(this.vehiclesByRoute, routeId, vehicle);
    }
    ...
    if (vehicle.currentStopSequence === undefined) {
      ...
      if (source === 'stop_id') {
        this.gaps.resolvedFromStopId++;
      } else if (source === 'derived') {
        this.gaps.stopSequenceDerived++;
      }
    }
    ...
    if (vehicle.currentStatus === 1) {
      ...
      if (stopId) {
        push(this.vehiclesAtStop, stopId, vehicle);
      }
    }
  }
  ...
  stopSequenceFor(vehicle: VehiclePosition): VehicleStopSequence | undefined {
    ...
    return sequence === undefined
      ? undefined
      : { sequence, source: 'derived', ambiguous: false };
  }
   ...
   */
  private stopSequenceFromStopId(
    vehicle: VehiclePosition
  ): VehicleStopSequence | undefined {
    if (!vehicle.stopId || !vehicle.tripId) {
      return undefined;
    }
    ...
    return {
      sequence: visits[0].stop_sequence,
      source: 'stop_id',
      ambiguous: visits.length > 1,
    };
  }
  ...
  private deriveStopSequence(tripId: string): number | undefined {
    ...
    for (const p of this.predictionsByTrip.get(tripId) ?? []) {
      ...
      if (p.time === undefined || p.time < floor) {
        continue;
      }
      ...
      const sequence =
        p.stop_sequence ??
        times?.find((t) => t.stop_id === p.stop_id)?.stop_sequence;
  ...
  upcomingAtStop(
    ...
    const future = list.filter(
      (p) => p.time === undefined || p.time >= nowSeconds - 60
    );
  ...
  upcomingAtStops(
    ...
    for (const id of stopIds) {
      all.push(...(this.predictionsByStop.get(id) ?? []));
    }
  ...
  nextAtStopsForRoute(
    ...
    for (const stopId of stopIds) {
      for (const p of this.predictionsByStop.get(stopId) ?? []) {
        if (p.time !== undefined && p.time < nowSeconds - 60) {
          continue;
        }
        ...
        if (dir === undefined || dir === null || String(dir) !== directionId) {
          continue;
        }
        // A prediction with no time cannot be ordered; it stands only until a
        // timed one turns up.
        if (!best || (p.time ?? Infinity) < (best.time ?? Infinity)) {
          best = p;
        }
      }
    }
