 ...
 *
 * A field label is the same three things wherever it is drawn: the field's
 * name, a badge saying how the reference talks about its presence, and a
 * hover/focus tooltip carrying the reference's own prose. What differs is only
 * where those facts come from - a schedule table's column, a realtime
 * message's field - so the presence vocabulary and the trigger markup live
 * here and the lookup stays with the spec layer each app owns.
 *
